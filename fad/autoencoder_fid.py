...
from fad import ann_ae, load_images
...
def get_autoencoder_frechet_distance(model, dataset, device, num_gen=5000):
    ...
    score = compute_autoencoder_frechet_distance(gen=sample_from_vae,
...
def compute_autoencoder_frechet_distance(dataset_name,
                                         gen=None,
                                         fdir=None,
                                         num_gen=5000,
                                         batch_size=256,
    ...
    stats = np.load(f'fad/stats/{dataset_name.lower()}_test_{latent_dim}.npz')
    ref_mu, ref_sigma = stats["mu"], stats["sigma"]

    feat_model.load_state_dict(
        torch.load(
            f'fad/stat_checkpoints/{dataset_name.lower()}_{latent_dim}.pth',
            map_location='cpu'))
    ...
    if gen is not None:
        ...
        dl = load_images.load_images(fdir, batch_size, grayscale=grayscale)
        dl = iter(dl)
        for idx in tqdm(range(num_iters)):
            img_batch = next(dl)
